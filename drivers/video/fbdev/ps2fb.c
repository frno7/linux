...
/* Module parameters */
static char *mode_option = "576x460i@50"; /* FIXME: PAL or NTSC from region */
...
struct ps2fb_par {
	...
	struct {
		...
		struct cb_tile {
			...
			u32 width2;	/* Least width in pixels, power of 2 */
			u32 height2;	/* Least height in pixels, power of 2 */
	...
	const struct fb_videomode *vm, const struct synch_gen sg)
{
	...
	return vm->yres <= 288 &&       bc ? vm_to_sp_sdtv(vm) :
...
static struct gs_sync_param vm_to_sp(const struct fb_videomode *vm)
{
	...
	for (i = 0; i < ARRAY_SIZE(preferred); i++) {
		...
		diff = abs(vm->pixclock -
			vck_to_pix_clock(gs_video_clock(t1248, lc, rc), spml));
