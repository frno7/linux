 ...
 *
 * Copyright (C) 2019 Fredrik Noring <noring@nocrew.org>
 *
...
static size_t package_sprite(union package *package,
	const struct fb_fillrect *region, const struct ps2fb_par *par)
{
	const struct gs_rgbaq rbaq = console_pseudo_palette(par, region->color);
	union package * const base_package = package;
...
void ps2fb_cb_fillrect(struct fb_info *info, const struct fb_fillrect *region)
{
	...
	spin_lock_irqsave(&par->lock, flags);
		...
		package += package_sprite(package, region, par);
