 ...
 *
 *	- RESET: The internal structure and purpose of this file are unknown.
 ...
 * this example, is therefore padded to 2032 bytes.
 */
 ...
 * @rom_extinfo_entry_unknown: FIXME
 */
...
static const struct rom_dir_entry * __init rom_dir_entry(
	const struct rom rom, loff_t offset, size_t entry_index)
{
	return rom_addr(rom,
		offset + entry_index * sizeof(struct rom_dir_entry),
	...
	const struct rom_dir_entry *extinfo)
{
	...
	if (!reset || !romdir || !extinfo) {
		pr_err("%s: Missing RESET, ROMDIR or EXTINFO\n", rom.name);
		return false;
	}
	...
	if (strcmp(reset->name, "RESET") != 0 ||
		...
		pr_err("%s: Misnamed RESET, ROMDIR or EXTINFO\n", rom.name);
		return false;
	}

	if (rom_align_file_size(reset->size) != offset) {
		pr_err("%s: Unaligned RESET %zu != %llu\n", rom.name,
			rom_align_file_size(reset->size), offset);
		return false;
	}

	if (romdir->size % sizeof(struct rom_dir_entry) != 0) {
		pr_err("%s: ROMDIR unaligned %zu\n", rom.name, romdir->size);
		return false;
	}
	...
	if (aligned_rom_header_size > rom.size) {
		pr_err("%s: ROM header to large %zu > %zu\n", rom.name,
	...
	const struct rom_dir_entry *extinfo)
{
	...
	if (!n) {
		pr_err("%s: Missing terminating entry\n", rom.name);
		return 0;
	}

	if (entries[n - 1].name[0] != '\0') {
		pr_err("%s: Nonterminating name: \"%s\"\n", rom.name,
			entries[n - 1].name);
		return 0;
	}
	...
	if (extinfo_size > extinfo->size) {
		pr_err("%s: EXTINFO too large %zu > %zu\n", rom.name,
			extinfo_size, extinfo->size);
		return 0;
	}

	if (size > rom.size) {
		pr_err("%s: ROM size too large %zu > %zu\n", rom.name,
			size, rom.size);
		return 0;
	}
...
static struct rom_dir __init rom_dir_for_rom(const struct rom rom)
{
	...
	size = rom_dir_size(rom, reset_offset, reset, romdir, extinfo);
	if (!size)
		return dir;
