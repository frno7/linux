...
static int ps2dma_make_tag_mem(unsigned long offset, int len, struct dma_tag_DEPRECATED **tagp, struct dma_tag_DEPRECATED **lastp, struct page_list *mem)
{
    struct dma_tag_DEPRECATED *tag;
    int sindex, eindex;
    ...
    tag = kmalloc(sizeof(struct dma_tag_DEPRECATED) * (eindex - sindex + 2), GFP_KERNEL);
    if (tag == NULL)
	return -ENOMEM;
...
static int ps2dma_make_tag_user(unsigned long start, int len, struct dma_tag_DEPRECATED **tagp, struct dma_tag_DEPRECATED **lastp, struct page_list **memp)
{
...
int ps2dma_make_tag(unsigned long start, int len, struct dma_tag_DEPRECATED **tagp, struct dma_tag_DEPRECATED **lastp, struct page_list **memp)
{
...
static unsigned long dma_stop(struct dma_request *req, struct dma_channel *ch)
{
    ...
    struct dma_tag_DEPRECATED *tag = ureq->tag;
    unsigned long eaddr;
    unsigned long maddr, qwc;
    struct dma_tag_DEPRECATED *taddr;
    ...
    taddr = (struct dma_tag_DEPRECATED *)bus_to_virt(READDMAREG(ch, PS2_Dn_TADR));
...
static int make_send_request(struct udma_request **ureqp,
			     ...
			     struct ps2_packet *pkt, struct dma_tag_DEPRECATED **lastp)
{
    struct udma_request *ureq;
    struct dma_devch *devch = &dev->devch[DMA_SENDCH];
    int result, len;
...
int ps2dma_send_list(struct dma_device *dev, int num, struct ps2_packet *pkts)
{
    ...
    struct dma_devch *devch = &dev->devch[DMA_SENDCH];
    struct dma_channel *ch = devch->channel;
    struct dma_tag_DEPRECATED *tag, *tag_bottom;
    int i;
    int result;
    ...
    if ((usreq->tag_head = (struct dma_tag_DEPRECATED *)get_zeroed_page(GFP_KERNEL)) /* TBD: check page reference counter */
	== NULL) {
	kfree(usreq);
	return -ENOMEM;
    }
    ...
    *(void **)usreq->tag_head = NULL;
	...
	if ((start & (DMA_TRUNIT - 1)) != 0 ||
		...
		*(void **)nextmem = NULL;
		...
		if (tag >= tag_bottom) {
		    struct dma_tag_DEPRECATED *nexthead, *nexttag;

		    if ((nexthead = (struct dma_tag_DEPRECATED *)get_zeroed_page(GFP_KERNEL)) == NULL) { /* TBD: check page reference counter */
			dma_sendl_free(&usreq->r.r, ch);
			return -ENOMEM;
		    }
