		       ...
		       struct pt_regs *regs, sigset_t *set)
{
	 ...
	 * struct sigframe.
	 */
	MIPS_WRITE_REG(regs->regs[ 4]) = ksig->sig;
	...
	MIPS_WRITE_REG(regs->regs[25]) = (unsigned long) ksig->ka.sa.sa_handler;
			  ...
			  struct pt_regs *regs, sigset_t *set)
{
	...
	err |= __save_altstack(&frame->rs_uc.uc_stack,
		MIPS_READ_REG_L(regs->regs[29]));
